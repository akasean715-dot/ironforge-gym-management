...
(() => {
  ...
  function reportModal(label){
    ...
    if(type.includes('profit')){const i=data.transactions.filter(t=>t.type==='Income' && transactionIsPaid(t)).reduce((a,t)=>a+Number(t.amount),0),e=data.transactions.filter(t=>t.type==='Expense' && transactionIsPaid(t)).reduce((a,t)=>a+Number(t.amount),0);content=`<div class="if-empty"><strong>Total income:</strong> ${money(i)}<br><strong>Total expenses:</strong> ${money(e)}<br><strong>Net:</strong> ${money(i-e)}</div>`;}
    ...
    else { const trainerRevenue=data.trainers.reduce((a,t)=>a+(Math.max(0,Number(t.members)||0)*Math.max(0,Number(t.monthlyFee ?? (Number(t.members)>0 ? Number(t.revenue||0)/Number(t.members) : 0))||0)),0); content=`<div class="if-empty">${data.trainers.length} trainers · ${data.trainers.reduce((a,t)=>a+(Number(t.members)||0),0)} assigned members · ${money(trainerRevenue)} monthly trainer revenue.</div>`; }
